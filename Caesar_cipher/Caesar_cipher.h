...
public:
    ...
    virtual const std::string& showResult() const;
...
protected:
    ...
    friend std::ostream& operator << (std::ostream& print,const ACaesarCipher& obj) {
        print << obj._m_line;
        return print;
    };
...
const std::string& ACaesarCipher::showResult() const {
    return _m_line;
}
...
const std::string& CodeCipher::coding (int __step) {
    try
    {
        ...
        std::cerr << e.what() << ": The entered value is less than 0:\n";
    }
}
